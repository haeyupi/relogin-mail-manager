...
- Web 控制台采用全宽工作区、状态卡片、账号列表和任务日志双栏布局。
- ZIP 导入区支持大尺寸拖拽上传、点击选择和文件名确认。
- Web 控制台支持搜索、状态筛选、单邮箱重登、批量重登、最近任务恢复、日志自动刷新、太阳/月亮主题切换和页面内修改登录密码。
...
2. 在 **导入与同步** 中拖入或选择原项目导出的 ZIP。
3. 在 **邮箱列表** 中搜索邮箱、手机号或失败原因。
4. 单个邮箱点击 **重登**。
5. 多个邮箱先勾选，再点击 **批量重登**。
6. 查看 **任务日志** 区域的进度条和日志；刷新页面后可以点击 **恢复最近日志**。
7. 顶栏的月亮/太阳按钮可以切换日间/夜间模式，显示偏好保存在当前浏览器。
8. 在 **目标配置** 中填写 **Web 登录密码** 后保存，可更新下一次 Basic Auth 使用的密码；留空不会覆盖当前密码。
